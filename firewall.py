...
def main():
    ...
    while True:
                        ...
                        if not any(
                            ...
                            try:
                                ...
                                if any(
                                    ...
                                    message = f"\t🚨 Blocked Subnet: 👉 {ip_address} {line.lower().split(" ")[5].split(" ")[0]}\n\t\t\t\t\t\t\t🔍 https://www.ip-tracker.org/lookup.php?ip={ip_address}\n"
        ...
        with open(WEB_HTML, "w") as f:
            f.write("<html><head><style> body { font-family: Arial, sans-serif; text-align: left; } #stats { font-size: 1em; margin-top: 50px; }</style></head>")
            f.write(
