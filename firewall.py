        ...
        with open(WEB_HTML, "w") as f:
            ...
            for line in activity:
                ...
                if not args.print and "🕵️" in line:
                    ...
                    URL_FIX = line.split(" ")
                    line = f"<a target=\"blank\" href=\"https://{URL_FIX[1]}\">🕵️ {URL_FIX[1]}</a> {URL_PARSE[1]} <a href=\"https://www.ip-tracker.org/lookup.php?ip=/{URL_PARSE[1]}\" target=\"_blank\">🔍 <a href=\"https://who.is/whois-ip/ip-address/{URL_PARSE[1]}\" target=\"_blank\">🌐</a>"
                if "\t" in line:
                    line.replace("\t", "")
                if "\n" in line:
                    line.replace("\n", "<br>")          
